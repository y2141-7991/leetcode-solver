...
impl Solution {
    pub fn uncommon_from_sentences(s1: String, s2: String) -> Vec<String>{
        ...
        let b:Vec<_> = frequency.iter()
            .filter_map(|(k, v)| if *v == 1 { Some((k, v)) } else { Some((&"", &2))}).collect();
...
fn main() {
    let a = Solution::uncommon_from_sentences("this apple is sweet".to_string(), "this apple is sour".to_string());
}
